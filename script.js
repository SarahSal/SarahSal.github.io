...
let cmsTableDemo = {
    'Bobby Jones': {
        'phone': '706-706-7060',
        'address': '1234 Bobby Avenue, NewYork, NY, 12345'
    },
    'George Romero': {
        'phone': '706-706-7160',
        'address': '1234 Bobby Avenue, NewYork, NY, 45678'
    }
};
...
let refreshDOMTable = () => {
    ...
    let tableContainer
    tableContainer = document.getElementById('cmsTableContainer');
    ...
    let newPersonSubmitBtn
    newPersonSubmitBtn = document.getElementById('newPersonSubmitBtn');
    let newPersonCancelBtn
    newPersonCancelBtn = document.getElementById('newPersonCancelBtn');
    ...
    for (let i = 0; i < editBtns.length; i++) {
        editBtns[i].addEventListener('click', ($event) => {
            let nameToEdit
            nameToEdit = $event.target.parentElement.children[0].innerText;
    ...
    for (let i = 0; i < deleteBtns.length; i++) {
        deleteBtns[i].addEventListener('click', ($event) => {
            ...
            let isSure = window.confirm('Are you sure want to delete ' + nameToDelete + '?');
            if (isSure)
                deleteUserFromTale(nameToDelete);
        })
    }

}

let deleteUserFromTale = (userName) => {
    let tempTable = {};
    let cmsTableKeys = Object.keys(cmsTable);
    for (let i = 0; i < cmsTableKeys.length; i++) {
